...
```
Query 1: Count visible buildings from 1 to 5
┌─────────────────────────────────────┐
│ Buildings: [1, 2, 3, 2, 1]        │
│ Heights:    1  2  3  2  1         │
│             ↑  ↑  ↑  ↑  ↑         │
│             ✓  ✓  ✓  ✗  ✗         │
│ Building 1: 1 > 0 ✓ (visible)     │
│ Building 2: 2 > 1 ✓ (visible)     │
│ Building 3: 3 > 2 ✓ (visible)     │
│ Building 4: 2 < 3 ✗ (blocked)     │
│ Building 5: 1 < 3 ✗ (blocked)     │
│ Count: 3 visible buildings         │
└─────────────────────────────────────┘
Result: 3

Query 2: Count visible buildings from 2 to 4
┌─────────────────────────────────────┐
│ Buildings: [1, 2, 3, 2, 1]        │
│ Heights:    1  2  3  2  1         │
│               ↑  ↑  ↑             │
│               ✓  ✓  ✗             │
│ Building 2: 2 > 0 ✓ (visible)     │
│ Building 3: 3 > 2 ✓ (visible)     │
│ Building 4: 2 < 3 ✗ (blocked)     │
│ Count: 2 visible buildings         │
└─────────────────────────────────────┘
Result: 2

Query 3: Count visible buildings from 3 to 5
┌─────────────────────────────────────┐
│ Buildings: [1, 2, 3, 2, 1]        │
│ Heights:    1  2  3  2  1         │
│                 ↑  ↑  ↑           │
│                 ✓  ✗  ✗           │
│ Building 3: 3 > 0 ✓ (visible)     │
│ Building 4: 2 < 3 ✗ (blocked)     │
│ Building 5: 1 < 3 ✗ (blocked)     │
│ Count: 1 visible building          │
└─────────────────────────────────────┘
...
Expected: 2, but I'm getting 1. Let me re-examine:
...
Wait, let me check if building 4 is actually visible:
Building 4 has height 2, and the maximum height so far is 3.
Since 2 < 3, building 4 is blocked by building 3.

But the expected output is 2... Let me re-read the problem.
Maybe I misunderstood the visibility rule.

Actually, let me trace through the algorithm again:
From building 3 to 5: [3, 2, 1]
- Building 3: height 3, max_so_far = 0, 3 > 0 ✓, max_so_far = 3
- Building 4: height 2, max_so_far = 3, 2 < 3 ✗
- Building 5: height 1, max_so_far = 3, 1 < 3 ✗
Count: 1

But expected is 2. Let me check if there's an error in my understanding...

Actually, let me just proceed with the visual example as is, noting the discrepancy.
```

**Visibility Algorithm Process:**
```
Step-by-step visibility checking:

Query 1: [1,5] → [1, 2, 3, 2, 1]
┌─────────────────────────────────────┐
│ Building 1: height=1, max=0, 1>0 ✓ │
│ Building 2: height=2, max=1, 2>1 ✓ │
│ Building 3: height=3, max=2, 3>2 ✓ │
│ Building 4: height=2, max=3, 2<3 ✗ │
│ Building 5: height=1, max=3, 1<3 ✗ │
│ Visible: 3 buildings                │
└─────────────────────────────────────┘

Query 2: [2,4] → [2, 3, 2]
┌─────────────────────────────────────┐
│ Building 2: height=2, max=0, 2>0 ✓ │
│ Building 3: height=3, max=2, 3>2 ✓ │
│ Building 4: height=2, max=3, 2<3 ✗ │
│ Visible: 2 buildings                │
└─────────────────────────────────────┘
...
Query [2,4]: Length = 3, k = 1
┌─────────────────────────────────────┐
│ Compare:                           │
│ - ST[1][2] = max(arr[2], arr[3]) = 3│
│ - ST[1][4-2^1+1] = ST[1][3] = 2   │
│ Result: max(3, 2) = 3              │
└─────────────────────────────────────┘
```
...
Query [2,4]: Find maximum in range [2,4]
┌─────────────────────────────────────┐
│ Query covers nodes:                 │
│ - Node [2,2]: max = 2              │
│ - Node [3,4]: max = 3              │
...
Sparse Table: O(n log n + q × n)
┌─────────────────────────────────────┐
│ Preprocessing: O(n log n)          │
│ Each query: O(n) scan (still)      │
│ Total: O(n log n + q × n)          │
└─────────────────────────────────────┘

Segment Tree: O(n log n + q × n)
┌─────────────────────────────────────┐
│ Preprocessing: O(n log n)          │
│ Each query: O(n) scan (still)      │
│ Total: O(n log n + q × n)          │
...
Note: Range maximum doesn't help with visibility counting
┌─────────────────────────────────────┐
│ Visibility requires sequential      │
│ processing to maintain max_so_far   │
│ Cannot be optimized with range      │
│ queries alone                       │
└─────────────────────────────────────┘
```
