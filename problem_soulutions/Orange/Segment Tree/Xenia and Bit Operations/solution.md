...
def solve():
    ...
    def build():
        ...
        for i in range(size - 1, 0, -1):
            ...
            tree[i] = tree[2 * i] | tree[2 * i + 1] if level % 2 == 1 else tree[2 * i] ^ tree[2 * i + 1]
    ...
    def update(pos, val):
        ...
        while pos >= 1:
            tree[pos] = tree[2 * pos] | tree[2 * pos + 1] if level % 2 == 1 else tree[2 * pos] ^ tree[2 * pos + 1]
            pos //= 2
            level += 1
...
def solve():
    ...
    def build(node, start, end, use_or):
        ...
        tree[node] = tree[2 * node] | tree[2 * node + 1] if use_or else tree[2 * node] ^ tree[2 * node + 1]
