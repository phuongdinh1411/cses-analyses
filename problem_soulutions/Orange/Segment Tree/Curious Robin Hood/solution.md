    ...
    for case in range(1, t + 1):
        ...
        def query_point(node, start, end, idx):
            if start == end:
                return tree[node]
            mid = (start + end) // 2
            return query_point(2 * node, start, mid, idx) if idx <= mid else query_point(2 * node + 1, mid + 1, end, idx)
...
def solve():
    ...
    for case in range(1, t + 1):
        ...
        def range_sum(l, r):
            return prefix_sum(r) if l == 0 else prefix_sum(r) - prefix_sum(l - 1)

        # Build BIT using enumerate
