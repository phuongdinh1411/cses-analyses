...
def solve():
    ...
    for _ in range(t):
        ...
        # Use list comprehension for reading preferences
        preferences = [int(input().strip(), 2) for _ in range(n)]

        min_dishes = k  # Worst case: need all dishes

        # Try all subsets of dishes (1 to 2^k - 1)
        for subset in range(1, 1 << k):
            # Check if all friends are satisfied using all()
            if all(pref & subset for pref in preferences):
                # Count dishes using bit_count() (Python 3.10+) or bin().count('1')
...
def solve():
    ...
    for _ in range(t):
        ...
        for subset in range(1, 1 << k):
            ...
            # Check if all friends satisfied using all()
            if all(pref & subset for pref in preferences):
                min_dishes = bits
...
def solve():
    ...
    for _ in range(t):
        ...
        # Try subsets of increasing size using any() for early exit
        for size in range(1, k + 1):
            # Use generator expression with any() for finding valid subset
