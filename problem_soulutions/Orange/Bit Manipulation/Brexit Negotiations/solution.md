...
def solve():
    ...
    for u in range(1, n + 1):
        ...
        base_time[u], d, *prereqs = line[0], line[1], *line[2:2+line[1]]
        for v in prereqs:
            deps[u].append(v)
            out_degree[v] += 1
    ...
    # Use list comprehension for initial heap
    heap = [(base_time[u], u) for u in range(1, n + 1) if out_degree[u] == 0]
    heapq.heapify(heap)
    ...
    while heap:
        ...
        max_meeting = max(max_meeting, dur + recap_time)
        recap_time -= 1
