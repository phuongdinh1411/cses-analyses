...
vector<tuple<int, int, bool>> adj[MAXN];  // (neighbor, edge_idx, is_a_side)
pair<int, int> edges[200005];
pair<int, int> result[200005];
bool visited[MAXN];
bool oriented[200005];
int n, m;
...
int dfs(int v) {
    ...
    for (auto& [neighbor, edge_idx, is_a_side] : adj[v]) {
        if (oriented[edge_idx]) {
            if (result[edge_idx].first == v) {
                out_count++;
            }
            continue;
        }
        ...
        if (!visited[neighbor]) {
            // Tree edge
            int child_out = dfs(neighbor);
            oriented[edge_idx] = true;
