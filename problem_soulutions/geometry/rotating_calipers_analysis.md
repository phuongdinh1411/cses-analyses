...
ll rotating_calipers_diameter_sq(vector<Point>& points) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        max_dist = max(max_dist, dist_sq(hull[ni], hull[j]));
    }
...
double min_area_rectangle(vector<Point>& points) {
    ...
    for (int i = 0; i < n; i++) {
        int ni = (i + 1) % n;
        Point edge = {hull[ni].first - hull[i].first,
                      hull[ni].second - hull[i].second};
        double edge_len = sqrt(edge.first*edge.first + edge.second*edge.second);

        // Find furthest point (top)
        while (cross(hull[i], hull[ni], hull[(top+1)%n]) >
               cross(hull[i], hull[ni], hull[top]))
            top = (top + 1) % n;

        // Find rightmost point
        if (i == 0) right = top;
        while ((hull[(right+1)%n].first - hull[i].first) * edge.first +
               (hull[(right+1)%n].second - hull[i].second) * edge.second >
               (hull[right].first - hull[i].first) * edge.first +
               (hull[right].second - hull[i].second) * edge.second)
            right = (right + 1) % n;

        // Find leftmost point
        if (i == 0) left = 0;
        while ((hull[(left+1)%n].first - hull[i].first) * edge.first +
               (hull[(left+1)%n].second - hull[i].second) * edge.second <
               (hull[left].first - hull[i].first) * edge.first +
               (hull[left].second - hull[i].second) * edge.second)
            left = (left + 1) % n;

        double height = abs(cross(hull[i], hull[ni], hull[top])) / edge_len;
        double width_r = ((hull[right].first - hull[i].first) * edge.first +
                         (hull[right].second - hull[i].second) * edge.second) / edge_len;
        double width_l = ((hull[left].first - hull[i].first) * edge.first +
                         (hull[left].second - hull[i].second) * edge.second) / edge_len;
        double width = width_r - width_l;
...
int main() {
    ...
    ll diameter_sq = rotating_calipers_diameter_sq(points);
    cout << fixed << setprecision(6) << sqrt(diameter_sq) << "\n";
