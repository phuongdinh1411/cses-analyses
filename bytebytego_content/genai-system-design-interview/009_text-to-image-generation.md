...
- **Remove small images:** We discard image–caption pairs where the image size is smaller than a certain threshold, for example, 64\xD764 pixels. Small images are often of poor quality and may not provide valuable information for training.
...
A diffusion model, as previously explained, progressively denoises a noisy image through multiple steps until it becomes clear. In each step, as illustrated in Figure 7, the model takes a noisy image as input and predicts the noise to be removed.[2](#user-content-fn-2) Two common architectures are typically used for this purpose:
...
As discussed in previous chapters, Inception score (IS) [27] and Fr\xE9chet Inception distance (FID) [28] are two common metrics for evaluating quality and diversity in image generation systems. In this section, we focus primarily on image–text alignment.
...
After training, similar text descriptions map close to each other in the embedding space, and images map near their relevant descriptions.[4](#user-content-fn-4)
...
- **Text alignment:**',(0,e.jsx)(A.em,{children:}),'}),'
...
[23] FSDP tutorial. [https://pytorch.org/tutorials/intermediate/FSDP_tutorial.html](https://pytorch.org/tutorials/intermediate/FSDP_tutorial.html).
...
[27] Inception score. [https://en.wikipedia.org/wiki/Inception_score](https://en.wikipedia.org/wiki/Inception_score).

[28] FID calculation. [https://en.wikipedia.org/wiki/Fr%C3%A9chet_inception_distance](https://en.wikipedia.org/wiki/Fr%C3%A9chet_inception_distance).
...
- In practice, we provide the diffusion model with additional conditioning inputs, such as the timestep. This will be discussed further in the training section. [↩](#user-content-fnref-1)
- For simplicity, we omitted the timestep input to the diffusion model. This will be covered in more detail in the training section. [↩](#user-content-fnref-2)
- For simplicity, we include only the noisy image, xt, and the timestep, t, as the input. As mentioned earlier, conditioning signals can also be included as inputs. [↩](#user-content-fnref-3)
- For simplicity, the embedding space is visualized in 2D. In reality, it is a d-dimensional space, where d represents the embedding size. [↩](#user-content-fnref-4)